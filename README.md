...
- 자동으로 쿠폰을 적용했더라도, 소비자는 쿠폰을 선택할 수 있다[^1]. <br />
- `정액 쿠폰`과 `정률 쿠폰`의 할인액이 같을 경우, `정액 쿠폰`[^2]으로 처리한다.

[^1]: 이렇게 하면 소비자가 직접 확인하여 더 저렴하게 구매한다고 느끼게 할 수 있지 않을까?
[^2]: 사실 결과는 같으나, 발생할 수 있는 예외 경우에 대해서는 이렇게 미리 정의해두는 편이 더 좋을듯싶다.
